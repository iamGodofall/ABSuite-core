...
| `ABSUITE_ADMIN_API_KEY` | Reads the record. 42 routes sit behind it. Without it the instance answers `/status` and returns 503 to everything else — perfectly healthy, reporting `UNKNOWN` across the board. |
